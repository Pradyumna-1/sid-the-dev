...
export default function ProjectCard({ imgSrc, title, description, repoLink, sourceLink, techIcons }) {
  // Ensure techIcons is an array and filter out any undefined or invalid icons
  const validTechIcons = Array.isArray(techIcons) ? techIcons.filter((item) => item && item.icon) : [];
  ...
  return (
    <div className="relative flex max-w-xs flex-col rounded-lg border border-gray-200 shadow-md dark:border-gray-700 dark:bg-neutral-800/70">
      ...
      <div className="p-5">
        ...
        {validTechIcons.length > 0 && (
          <div className="mt-5 mb-7 flex flex-wrap gap-5">
            {validTechIcons.map((iconObj, i) => {
              if (iconObj && iconObj.icon) {
                return <Iconify key={`icon-${i}`} classes="text-2xl opacity-80" icon={iconObj.icon} />;
              }
              return null; // Skip rendering if there's no valid icon
            })}
          </div>
        )}
      </div>
